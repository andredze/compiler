...
#ifdef BACKEND
    enum LangErr (*asm_function) (struct LangCtx*, TreeNode_t*);
...
#define SET_OP_CASE_(code,          repeat_times,   name,                                     ast_format,                                                   asm_function,   asm_name) \
        [(code)] = {(code), #code, (repeat_times), (name), sizeof(name) / sizeof(*name) - 1, (ast_format), (enum LangErr (*)(struct LangCtx*, TreeNode_t*))(asm_function), (asm_name)}
...
{
//                      code,         repeat_times,         name                  ast_format                asm_function      asm_name
    SET_OP_CASE_(OP_NONE                ,   1,   L""                            , L""               , NULL                   , L""    ),
    SET_OP_CASE_(OP_CMD_SEPARATOR       ,   1,   L"ЗАМОЛЧИ"                     , L"_END_STATEMENT_", AssembleCmdSeparator   , L""    ),
    SET_OP_CASE_(OP_BRACKET_OPEN        ,   1,   L"("                           , L""               , NULL                   , L""    ),
    SET_OP_CASE_(OP_BRACKET_CLOSE       ,   1,   L")"                           , L""               , NULL                   , L""    ),
    SET_OP_CASE_(OP_ASSIGNMENT          ,   1,   L"пересади в"                  , L"_ASSIGNMENT_"   , AssembleAssignment     , L""    ),
